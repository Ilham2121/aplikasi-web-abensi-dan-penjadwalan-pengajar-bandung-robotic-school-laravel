    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h4 class="mb-sm-0">Edit Schedule</h4>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">Edit Schedule Information</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.schedules.update', $schedule->id) }}" method="POST">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="time" class="form-control @error('start_time') is-invalid @enderror" id="start_time" name="start_time" value="{{ old('start_time', date('H:i', strtotime($schedule->start_time))) }}" required>
                                @error('start_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="time" class="form-control @error('end_time') is-invalid @enderror" id="end_time" name="end_time" value="{{ old('end_time', date('H:i', strtotime($schedule->end_time))) }}" required>
                                @error('end_time')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('schedule_type') is-invalid @enderror" id="schedule_type" name="schedule_type" required>
                                    ...
                                    @foreach ($scheduleTypes as $type)
                                        <option value="{{ $type }}" {{ old('schedule_type', $schedule->schedule_type) == $type ? 'selected' : '' }}>
                                            {{ ucfirst($type) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('semester') is-invalid @enderror" id="semester" name="semester" required>
                                    ...
                                    @foreach ($semesters as $semester)
                                        <option value="{{ $semester }}" {{ old('semester', $schedule->semester) == $semester ? 'selected' : '' }}>
                                            {{ ucfirst($semester) }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <select class="form-select @error('academic_year') is-invalid @enderror" id="academic_year" name="academic_year" required>
                                    ...
                                    @foreach ($academicYears as $year)
                                        <option value="{{ $year }}" {{ old('academic_year', $schedule->academic_year) == $year ? 'selected' : '' }}>
                                            {{ $year }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control @error('semester_start') is-invalid @enderror" id="semester_start" name="semester_start" value="{{ old('semester_start', date('Y-m-d', strtotime($schedule->semester_start))) }}" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" class="form-control @error('semester_end') is-invalid @enderror" id="semester_end" name="semester_end" value="{{ old('semester_end', date('Y-m-d', strtotime($schedule->semester_end))) }}" required>
                                @error('semester_end')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
